...
def add_note():
    ...
    def save_note():
        ...
        # Create a text widget with the content to display in the notebook
        note_content = tk.Text(notebook, width=40, height=10)
        note_content.insert(tk.END, content.strip())
        note_content.config(state=tk.DISABLED)  # Make it non-editable after save
        
        # Update the tab to display the saved content
        notebook.forget(notebook.select())  # Remove the 'New Note' tab
        notebook.add(note_content, text=title)  # Add the note with its title
        
    # Add a save button to the note frame
    save_button = ttk.Button(note_frame, text="Save", command=save_note)
    save_button.grid(row=2, column=1, padx=10, pady=10)

# Load saved notes into the notebook
def load_notes():
    try:
        with open("notes.json", "r") as f:
            notes = json.load(f)
        ...
        for title, content in notes.items():
            ...
            note_content.config(state=tk.DISABLED)  # Make it non-editable
